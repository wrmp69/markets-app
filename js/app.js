...
function dayCard(date, day, deletable){
  ...
  const summary=`${groups.length} exercice(s) · ${entries.length} série(s) · ${cardio.length} cardio · ${Math.round(vol)} kg${prCount?` · ${prCount} PR`:''}`;
  ...
  return `<article class="item history-day ${open?'is-open':''}"><div class="between history-day-head"><button class="history-day-main" data-action="history-day-toggle" data-date="${date}"><div><div class="item-title">${fmtDateLong(date)}</div><div class="meta">${summary}</div></div><span class="history-day-chevron">${open?'⌄':'›'}</span></button><div class="row"><button class="btn small secondary" data-action="history-day-toggle" data-date="${date}">${open?'Masquer':'Voir la séance du jour'}</button>${deletable?`<button class="btn small danger" data-action="day-delete" data-date="${date}">Suppr.</button>`:''}</div></div>${expanded}</article>`;
}
